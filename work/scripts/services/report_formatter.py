...
def _row(label: str, value: object, indent: int = 4) -> str:
    """Format a single report line with aligned label and value.
...
def _section(title: str) -> str:
    """Return a section header with a separator line.
...
class ReportFormatter:
    ...
    def format(
        ...
        """Build and return the full plain-text analytics report.
    ...
    def _header(self, df: pd.DataFrame) -> str:
        """Build the header with timestamp and data period.
    ...
    def _descriptive(self, eda: "EDAReportResult") -> str:
        """Build the descriptive statistics section.
        ...
        for col, r in eda.descriptive.items():
            ...
            lines.append(
                _row("Усечённое среднее", f"{r.trimmed_mean:.6f}")
            )
            lines.append(_row("МАО", f"{r.mad:.6f}"))
            lines.append(
                _row("КВ", f"{r.coefficient_of_variation:.6f}")
            )
            lines.append(
                _row(
                    "Пропущено",
                    f"{r.missing_count}  ({r.missing_ratio:.2%})"
                )
            )
            lines.append(
                _row(
    ...
    def _timeseries(self, ts: "TSReportResult") -> str:
        """Build the time-series diagnostics section (ADF + ACF).
        ...
        for col, report in ts.reports.items():
            ...
            if report.stationarity is not None:
                ...
                lines.append(_row(
                    "Статистика ADF",
                    f"{s.result.statistic:.6f}"
                ))
                lines.append(_row(
                    "p-значение",
                    f"{s.result.pvalue:.6f}"
                ))
                lines.append(_row(
                    "Лагов использовано",
                    str(s.result.used_lags)
                ))
                lines.append(_row(
                    "Наблюдений", str(s.result.nobs)
                ))
                lines.append(_row("Гипотеза", s.hypothesis))
                lines.append(_row(
                    "Стационарен",
                    "Да" if s.passed else "Нет"
                ))
                lines.append(_row(
                    "Рекомендация", s.recommendation
                ))
            ...
            if report.autocorrelation is not None:
                ...
                lines.append(_row(
                    "Сильнейший лаг ACF",
                    str(a.strongest_acf_lag)
                ))
                lines.append(_row(
                    "Сильнейший лаг PACF",
                    str(a.strongest_pacf_lag)
                ))
                ...
                acf_str = (
                    ", ".join(str(lag) for lag in sig_acf[:10]) or "нет"
                )
                pacf_str = (
                    ", ".join(str(lag) for lag in sig_pacf[:10]) or "нет"
                )
    ...
    def _correlation(self, eda: "EDAReportResult") -> str:
        """Build the top correlations section.
        ...
        if corr.top_positive:
            ...
            for p in corr.top_positive[:5]:
                lines.append(
                    f"    {p.feature_x:<20} x  {p.feature_y:<20}"
    ...
    def _failures(self, ts: "TSReportResult") -> str:
        """Build the analysis failures section.
